...
export function parsePolygon(
    ...
    for (const [geometryIndex, offset] of offsets.entries()) {
      ...
      const polygon = extrudePolygon([data], {depth: heights[geometryIndex] / metersPerUnit, excludeBottom: true});
  
      for (let index = 0; index < polygon.position.length / 3; ++index) {
        [polygon.position[3 * index + 1], polygon.position[3 * index + 2]] = [polygon.position[3 * index + 2], polygon.position[3 * index + 1]];
        [polygon.normal[3 * index + 1], polygon.normal[3 * index + 2]] = [polygon.normal[3 * index + 2], polygon.normal[3 * index + 1]]
      }
      
      positions.push(...Array.from(polygon.position));
      normals.push(...Array.from(polygon.normal));
      indices.push(...Array.from(polygon.indices).map((x: number) => x + positionOffset / 3));
      faceMapping.push(...new Array(polygon.position.length / 3).fill(ids[geometryIndex]));
