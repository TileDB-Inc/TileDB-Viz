...
registerAssetFactory(
  ...
  async (
    ...
    const image = opts.token
      ? await fetch(opts.uri, {
          headers: { Authorization: `Bearer ${opts.token}` }
        })
          .then(response => response.arrayBuffer())
          .then(buffer => new File([buffer as BlobPart], 'image.svs'))
          .then(file => openslide.open(file))
      : await openslide.open(opts.uri);
    const extent = await image.getLevelDimensions(
      (await image.getLevelCount()) - 1
    );
    const extent_max = await image.getLevelDimensions(0);
    ctx.sceneOptions.extents.encapsulateBoundingInfo(
      new BoundingInfo(
        new Vector3(0, 0, 0),
        new Vector3(extent[0], extent[1], 0)
      )
    );

    const properties = await image.getPropertyNames();
    const defaultScaleX = Number.parseFloat(
      (await image.getPropertyValue(
        'aperio.MPP' in properties ? 'aperio.MPP' : 'openslide.mpp-x'
      )) ?? '1'
    );
    const defaultScaleY = Number.parseFloat(
      (await image.getPropertyValue(
        'aperio.MPP' in properties ? 'aperio.MPP' : 'openslide.mpp-y'
      )) ?? '1'
    );
    ...
    const metadata: SVSImageMetadata = {
      ...
      uri: opts.uri,
      name: opts.name ?? '',
      root: constructImageTileset(
        await Promise.all(
          Array.from({ length: await image.getLevelCount() }, (_, i) => i)
            .reverse()
            .map(async i => {
...
registerAssetFactory(
  ...
  async (
    ...
    if (opts.defaultChannels) {
      const defaultAttribute = metadata.attributes.filter(x => x.visible)[0]
        ?.name;
