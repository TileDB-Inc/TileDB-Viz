import { MoctreeBlock } from '../octree/moctree';
...
export class PriorityQueue {
    ...
    constructor(maxSize: number) {
        this.heap = Array.from(Array(maxSize), (_, index) => ({
            score: -1,
            octreeBlock: null
        } as Block));
    }
    ...
    // in the Binary Heap
    insert(score: number, octreeBlock: MoctreeBlock) 
    {
        this.size += 1;
        this.heap[this.size] = {score: score, octreeBlock: octreeBlock} as Block;
    ...
    changePriority(index: number, score: number) {
        var oldscore = this.heap[index].score;
