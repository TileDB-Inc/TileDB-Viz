import { Scene, SceneLoader, Vector3, Space, ISceneLoaderAsyncResult } from '@babylonjs/core';
import {
...
class PointCloudGUI {
	...
	public async init(scene: Scene, model: ArrayModel) {
		...
		var fileStackPanel = new StackPanel('stackPanel');
		...
		var namespaceLabel = new TextBlock("inputLabel", 'Namespace');
		...
		var namespaceInput = new InputText('namespaceInput');
		...
		var fileLabel = new TextBlock("fileLabel", 'File');
		fileLabel.width = 1;
		...
		var fileInput = new InputText('fileInput');
		...
		var positionGrid = new Grid('positionGrid');
		...
		var positionXLabel = new TextBlock("positionXLabel", 'Translation X');
		...
		var positionYLabel = new TextBlock("positionYLabel", 'Translation Y');
		...
		var positionZLabel = new TextBlock("positionZLabel", 'Translation Z');
		...
		var scaleLabel = new TextBlock("scaleLabel", 'Scale');
		scaleLabel.width = 1;
		...
		var positionXInput = new InputText('positionXInput');
		...
		var positionYInput = new InputText('positionYInput');
		...
		var positionZInput = new InputText('positionZInput');
		...
		var scaleInput = new InputText('scaleInput');
		scaleInput.height = 1;
		...
		var loadButton = Button.CreateSimpleButton('loadButton', 'Load Model');
		loadButton.width = 1;
		...
		loadButton.onPointerUpObservable.add(() => 
		{
			...
			client.getFileContents(namespaceInput.text, fileInput.text).then((response: { buffer: ArrayBuffer; originalFileName: string; }) => {
				SceneLoader.ImportMeshAsync("", "", new File([response.buffer], response.originalFileName)).then((result: ISceneLoaderAsyncResult) =>
					{
