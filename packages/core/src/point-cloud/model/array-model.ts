...
import { TileDBWorkerPool, TileDBWorkerPoolHeapTree, TileDBWorkerPoolMoctree } from '../workers';
import Octree from '../octree/octree';
import { HeapTree, HTBlock } from '../octree/heaptree';

/**
 * The ArrayModel manages the client octree
 */
class ArrayModel<BlockType> {
  groupName?: string;
  namespace?: string;
  octree!: Octree<BlockType>;
  bufferSize: number;
  rgbMax!: number;
  ...
  protected loadSystem(block: BlockType) {
    throw Error(
      'method loadSystem in ArrayModel for block type: ' +
        typeof block +
        ' not implemented'
    );
  }

  protected async fetchBlock(block: BlockType | undefined) {
    throw Error(
      'method fetchBlock in ArrayModel for block type: ' +
        typeof block +
        ' not implemented'
    );
  }
  ...
  public async init(
    ...
    throw Error('method init in ArrayModel for this block type not implemented');
  }

  public async fetchPoints(scene: Scene) {
    throw Error('method fetchPoints in ArrayModel for this block type not implemented');
  }

  set metadata(m: Map<string, number>) {
    throw Error('method set metadata in ArrayModel for this block type not implemented');
  }

  public afterRender(scene: Scene) {
    throw Error('method afterRender in ArrayModel for this block type not implemented');
  }
}

class MoctreeArrayModel extends ArrayModel<MoctreeBlock> {
  octree!: Moctree;
  workerPool?: TileDBWorkerPoolMoctree | undefined;
  constructor(options: TileDBPointCloudOptions) {
    super(options);
  }
  protected loadSystem(block: MoctreeBlock) {
  ...
  protected async fetchBlock(block: MoctreeBlock | undefined) {
  ...
  public async init(
    ...
    if (data) {
      ...
      this.workerPool = new TileDBWorkerPoolMoctree(
        {
...
class HeapTreeArrayModel extends ArrayModel<HTBlock> {
  octree!: HeapTree;
  workerPool?: TileDBWorkerPoolHeapTree | undefined;
  constructor(options: TileDBPointCloudOptions) {
    super(options);
  }
  protected loadSystem(block: HTBlock) {
    if (block.entries !== undefined && this.scene) {
      if (!this.particleSystems.has(block.code) || !this.basePcs) {
        console.log(
          'received: ' +
            block.entries?.X.length +
            ' of approximately: ' +
            this.octree.knownBlocks.get(block.code)
        );
        ...
        if (this.useSPS) {
          ...
          if (block.code !== HeapTree.startBlockIndex) {
            this.particleSystems.set(block.code, sps);
          } else {
            this.basePcs = sps;
          }
        } else {
          ...
          pcs.buildMeshAsync().then(() => {
            pcs.setParticles();
            if (block.code !== HeapTree.startBlockIndex) {
              this.particleSystems.set(block.code, pcs);
            } else {
              this.basePcs = pcs;
            }
          });
        ...
        // lru cache - reinsert this pcs
        if (block.code !== HeapTree.startBlockIndex) {
          const pcs = this.particleSystems.get(block.code);
          if (pcs) {
            this.particleSystems.delete(block.code);
            this.particleSystems.set(block.code, pcs);
          }
        }
      }
  ...
  protected async fetchBlock(block: HTBlock | undefined) {
    // fetch if not populated
    if (block) {
      if (!block.entries) {
        this.workerPool?.postMessage({
  ...
  public async init(
    ...
    const spanY = (ymax - ymin) / 2.0;
    this.translationVector = new Vector3(xmin + spanX, zmin, ymin + spanY);
    this.octree = new HeapTree(
      new Vector3(-spanX, 0, -spanY),
      new Vector3(spanX, zmax - zmin, spanY),
      this.maxLevel,
      this.fanOut
    );
    this.neighbours = this.octree.getNeighbours(HeapTree.startBlockIndex);
    ...
    if (data) {
      ...
      this.workerPool = new TileDBWorkerPoolHeapTree(
        {
          type: WorkerType.init,
          namespace: this.namespace,
          token: this.token,
    ...
    if (this.basePcs && this.basePcs.nbParticles > 0) {
      ...
      if (scene.activeCamera && this.workerPool?.isReady()) {
        ...
        if (!ray.origin.equalsWithEpsilon(this.rayOrigin, epsilon)) {
          ...
          if (parentBlocks.length > 0) {
            const pickCode = parentBlocks[0].code;
            this.pickedBlockCode = pickCode;
            this.renderBlocks = parentBlocks;
            this.isBuffering = false;
            this.neighbours = this.octree.getNeighbours(this.pickedBlockCode);
  ...
  set metadata(m: Map<string, number>) {
    m.forEach((v, k) => {
      this.octree.knownBlocks.set(parseInt(k), v);
    });
  }
...
export { ArrayModel, MoctreeArrayModel, HeapTreeArrayModel };
