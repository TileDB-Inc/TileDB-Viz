...
<script lang="ts">
  ...
  function featureOnChange(index: number) {
    ...
    if (feature.type === 2) {
      state.options[state.selectedDataset].selectedAttribute = 
      attributes[state.selectedDataset].find(x => x.name === feature.attributes[0]);
    }
...
<Section>
  <div slot="header" class="Viewer-PointCloud__title">
    ...
    <Slider id={'pointBudget'} label={'Point budget'} min={100_000} max={10_000_000} value={state.options[state.selectedDataset].pointBudget} step={10_000} callback={pointBudgetOnChange} formatter={value => `${(value / 1_000_000).toFixed(1)}M`}/>
		<Slider id={'quality'} label={'Quality'} min={1} max={50} value={state.options[state.selectedDataset].quality} step={1} callback={qualityOnChange} formatter={value => value.toFixed(0)}/>
    ...
    {:else}
      ...
      {:else if currentFeature.type === 2}
        <CategoricalPanel 
          state={categoryState[targets[state.selectedDataset][0]][state.options[state.selectedDataset].selectedAttribute.enumeration]} 
