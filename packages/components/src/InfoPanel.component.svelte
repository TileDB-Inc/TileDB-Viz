...
<script>
  ...
  function itemsPerPageOnChange(event) {
    itemsPerPage = Number.parseInt(event.target.value);
  }

  function pageOnChange(event) {
    page = Math.max(
      0,
      Math.min(pages - 1, page + Number.parseInt(event.target.value))
    );
  }
  ...
  function itemOnSelect(page, index) {
  ...
  onDestroy(() => {
    window.removeEventListener(events.ENGINE_INFO_UPDATE, itemOnPick, {
      capture: true
    });
  });
</script>
    ...
    <div class="Viewer-DataTable__horizontal-container">
      <div class="Viewer-DataTable__pagination-control">
        ...
        <div class="Viewer-DataTable__select">
          <select name="itemsPerPage" on:change={itemsPerPageOnChange}>
...
<style lang="scss">
  .Viewer-DataTable {
    ...
    &__row-selected {
      box-shadow: inset 0 0 8px var(--viewer-accent);
    }
  }
