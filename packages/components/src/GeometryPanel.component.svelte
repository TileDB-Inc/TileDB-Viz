...
<script>
  ...
  import TypedInput from './TypedInput.component.svelte';
  ...
  function loadPickedObject(e) {
    if (e.detail.type === 'GEOMETRY_INFO_DATA') {
      selectedGeometry = e.detail.info;
    }
  }
  ...
  let attributeMap = new Map(JSON.parse(attributes ?? '[]').map(item => [item.name, item]));
</script>
      ...
      <div class="Viewer-GeometrySelector__pickInfo">
        {#each Object.entries(selectedGeometry).sort((a,b) => (a[0] > b[0]) ? 1 : ((b[0] > a[0]) ? -1 : 0)) as [key, value]}
          {#if attributeMap.has(key)}
            <TypedInput attribute={attributeMap.get(key)} value={value}></TypedInput>
          {/if}
        {/each}
        <div class="Viewer-GeometrySelector__controls">
          <button class="Viewer-GeometrySelector__button">
            Save
          </button>
          <button class="Viewer-GeometrySelector__button" on:click={() => clearSelectedGeometry()}>
            Cancel
          </button>
        </div>
      </div>
    {:else}
      <div class="Viewer-GeometrySelector__text">
