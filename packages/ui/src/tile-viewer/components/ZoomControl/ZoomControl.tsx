...
import pprintZoom from '../../utils/pprintZoom';
import Tooltip from '../Tooltip';
import Section from '../Section';
import './ZoomControl.scss';
import { TileViewerEvents, ZoomEvent } from '@tiledb-inc/viz-core';
...
const ZoomControl: React.FC = props => {
  ...
  const zoomLevel = React.useMemo(() => pprintZoom(zoom), [zoom]);

  const dispatchZoomEvent = React.useCallback((zoom: number) => {
    window.dispatchEvent(new CustomEvent<ZoomEvent>(TileViewerEvents.ZOOM, {
      bubbles: true,
      detail: { zoom: zoom } as ZoomEvent,
    }));
  }, []);

  return (
    <div className="ImageViewer-ZoomControl">
      <Section
        title={
          <>
            <svg
      ...
      >
        <div className="ImageViewer-ZoomControl__group">
          <Tooltip tooltipText="Zoom in +">
            <button
              className="ImageViewer-ZoomControl__button"
              onClick={zoomIn}
            >
              <svg
                width="20"
          ...
          <p className="ImageViewer-ZoomControl__label">{zoomLevel}</p>
        </div>
        <div className="ImageViewer-ZoomControl__group">
          <p className="ImageViewer-ZoomControl__label">Reset view</p>
          <Tooltip tooltipText="Reset zoom level">
            <button
              className="ImageViewer-ZoomControl__button"
              onClick={resetZoom}
            >
