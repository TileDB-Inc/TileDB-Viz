...
import { LevelRecord, Attribute, types } from '@tiledb-inc/viz-core';
import Tooltip from '../Tooltip';
import { clearCache, getTileCount } from '@tiledb-inc/viz-core';
import './CacheControl.scss';
...
export const CacheControl: React.FC<Props> = (props: Props) => {
  const [state, setState] = useState<CacheControlState>({
    tileCount: 0,
    size: 0
  });

  const clear = async () => {
    await clearCache(props.levels.map(x => `${x.id}_${props.tileSize}`));
  };

  const caclulateCacheInfo = async () => {
    if (props.attribute) {
      const tileCount = await getTileCount(
        props.levels.map(x => `${x.id}_${props.tileSize}`)
      );
      const size =
        (tileCount *
          (types as any)[props.attribute.type.toLowerCase()].bytes *
          props.tileSize ** 2) /
        2 ** 30;
      setState(st => ({ ...st, tileCount: tileCount, size: size }));
    }
  };
  ...
  }, [props.levels, props.attribute]);

  return (
    <div className="ImageViewer-CacheControls">
      <Section
        title={
          <>
            <svg
      ...
      >
        <div className="ImageViewer-CacheControls__container">
          <div className="ImageViewer-CacheControls__group">
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
            ...
            <p className="ImageViewer-CacheControls__label">
              Cached tiles: {state.tileCount}
            </p>
            <Tooltip tooltipText="Clear cache">
              <button
                className="ImageViewer-CacheControls__button"
                onClick={clear}
              >
                <svg
                  width="20"
