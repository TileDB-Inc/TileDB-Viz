...
interface GroupSelectorState {
  ...
  filteredGroups: Array<ImageGroup>;
  searchTerm: string;
  pages: number;
  currentPage: number;
  isOpen: boolean;
...
const GroupSelector: React.FC<Props> = props => {
  const { groupNamespace, baseGroup, client } = props;
  ...
  useEffect(() => {
    client.groups
      .getGroupContents(groupNamespace, baseGroup)
      .then((value: GroupContents) => {
        if (value.entries) {
          const groups = value.entries
          ...
          setState(st => ({
            ...
            pages: Math.ceil(groups.length / props.itemsPerPage)
          }));
        }
      });
  }, [
    props.groupNamespace,
    props.baseGroup,
    client.groups,
    groupNamespace,
    baseGroup,
  ...
  const onPreviousPage = useCallback(() => {
    setState(st => ({
      ...st,
      currentPage: Math.max(0, st.currentPage - 1)
    }));
  }, []);
  ...
  const onNextPage = useCallback(() => {
    setState(st => ({
      ...st,
      currentPage: Math.min(st.pages - 1, st.currentPage + 1)
    }));
  }, []);
  ...
  const filteredGroups = useMemo(() => {
    ...
    setState(st => ({
      ...st,
      currentPage: 0,
      pages: Math.ceil(filteredGroups.length / props.itemsPerPage)
    }));

    return filteredGroups;
  }, [props.itemsPerPage, state.groups, state.searchTerm]);

  const shouldRenderPagination = useMemo(
    () => props.itemsPerPage <= filteredGroups.length,
    [filteredGroups.length, props.itemsPerPage]
  );

  return (
    <div className="ImageViewer-GroupSelector">
      <Section
      ...
      >
        <div className="ImageViewer-GroupSelector__content">
          <input
            className="ImageViewer-GroupSelector__value"
            value={state.searchTerm}
            onChange={e =>
              setState(st => ({ ...st, searchTerm: e.target.value }))
            }
            placeholder="Search Group"
            type="search"
          ></input>
          <ul className="ImageViewer-GroupSelector__list">
            {filteredGroups
              .slice(
                state.currentPage * props.itemsPerPage,
                (state.currentPage + 1) * props.itemsPerPage
              )
              .map((item, _) => {
                return (
                  <li
                    key={item.id}
                    className="ImageViewer-GroupSelector__item"
                    onClick={() => props.onGroupSelect(item.id)}
                  >
                    <div className="BioImageViewer-GroupSelector__item-icon">
                      <svg
                        width="32"
                        height="32"
                        viewBox="0 0 32 32"
                    ...
                    <div className="ImageViewer-GroupSelector__item-main">
                      <p className="ImageViewer-GroupSelector__item-namespace">
                        <span className="ImageViewer-GroupSelector__item-text">
                          {item.namespace}
                        </span>
                      </p>
                      <p className="ImageViewer-GroupSelector__item-name">
                        <span className="ImageViewer-GroupSelector__item-text">
                          {item.name}
                        </span>
                      </p>
          ...
          {shouldRenderPagination && (
            <div className="ImageViewer-GroupSelector__controls">
