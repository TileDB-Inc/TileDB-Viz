...
const Header: React.FC<Props> = (props: Props) => {
  const { children, toggleVisibility, visibleContent } = props;
  return (
    <h5 className="ImageViewer-Header">
      <span>{children}</span>
      <button
        className="ImageViewer-Header__button"
        onClick={toggleVisibility}
      >
        <svg
          className={
            visibleContent ? '' : 'ImageViewer-Header__icon--rotated'
          }
          width="20"
          height="20"
          viewBox="0 0 20 20"
